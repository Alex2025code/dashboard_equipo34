...
# Sección 3: Distribución de Calificaciones(Histograma)
st.subheader("Distribución de Calificaciones")
fig3, ax3 = plt.subplots(figsize=(10, 4))
sns.histplot(filtered_df['Rating'], bins=10, kde=True, ax=ax3)
ax3.set_title("Distribución de Calificaciones de Clientes")
...
# Sección 5: COGS vs Ingreso Bruto (diagrama de dispersión)
...
for container in ax6.containers:
    ax6.bar_label(container, fmt='%d', label_type='edge', fontsize=9)
st.pyplot(fig6)
...
sns.heatmap(
    filtered_df.corr(numeric_only=True), # solo columnas numéricas
    annot=True, cmap="coolwarm",
    ax=ax7)
ax7.set_title("Matriz de Correlación Numérica")
st.pyplot(fig7)

# Sección 8: Composición del Ingreso Bruto por Sucursal
...
pivot_data = grouped.pivot(index='Branch', columns='Product line', values='Total').fillna(0)
#Crear gráfico
